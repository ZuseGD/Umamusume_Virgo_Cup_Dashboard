...
def show_view(df, team_df):
    ...
    if not filtered_team_df.empty:
        ...
        st.info("Need more data for Teams.")
    ...
    st.plotly_chart(style_fig(fig_style), use_container_width=True, config=PLOT_CONFIG)
    ...
    st.plotly_chart(style_fig(fig_runner, height=400), use_container_width=True, config=PLOT_CONFIG)